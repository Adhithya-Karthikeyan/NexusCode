...
| `--effort <lvl>` | | Reasoning effort. `off` always works; every other value is PROVIDER-NATIVE — run `nexus effort <provider>` to see the real, live-probed scale (claude-code alone accepts seven: `low\|medium\|high\|xhigh\|max\|ultracode\|auto`; codex's set is pulled from its own configured model). Left unset, a token-budget HTTP provider (anthropic/gemini/vertex/bedrock) gets an implicit "medium" default so extended thinking is genuinely on; claude-code/codex are left alone so their own already-configured effort governs. `ask`/`agent`/`chat`/`compare`/`race`/`consensus`/`chain`/`plan`/`code`. A provider that can't honor an EXPLICIT `--effort` warns on stderr and runs without it — never silently. |
...
| `--effort <lvl>` | string | `config.defaultEffort` (`off`) | "off" always works; every other value is PROVIDER-NATIVE (see `nexus effort <provider>`); unsupported providers warn on stderr and run without it |
...
| `--effort <lvl>` | string | `config.defaultEffort` (`off`) | "off" always works; every other value is PROVIDER-NATIVE (see `nexus effort <provider>`) (both paths); unsupported providers warn on stderr and run without it. A `--role` run against a cli-subprocess agent is refused before `--effort` is evaluated. |
| `-p, --provider <id>` | string | config default | Provider to run against |
| `-m, --model <id>` | string | provider default | Model id |
| `-s, --system <text>` | string | built-in prompt | System prompt |
| `-o, --output <mode>` | enum | `text` | `text`, `json`, or `ndjson` |
...
| `-m, --model <id>` | string | provider default | Model id |
| `-o, --output <mode>` | enum | `text` | `text`, `json`, or `ndjson` |
| `--effort <lvl>` | string | `config.defaultEffort` (`off`) | "off" always works; every other value is PROVIDER-NATIVE (see `nexus effort <provider>`), resolved once for the whole session; unsupported providers warn on stderr and run without it |
...
| `-s, --system <text>` | string | built-in prompt | System prompt |
| `-o, --output <mode>` | enum | `text` | `text`, `json`, or `ndjson` |
| `--effort <lvl>` | string | `config.defaultEffort` (`off`) | Reaches the CLI for real: claude-code wires it as `--effort <level>` (its own session flag), codex wires it as `-c model_reasoning_effort=<level>`. Run `nexus effort <agent>` for the real, live-probed scale. With no `--effort`, each CLI's own already-configured effort governs, untouched. |
...
| `-o, --output <mode>` | enum | `text` | `text`, `json`, or `ndjson` |
| `--cwd <dir>` | string | current dir | Working directory for tools and jobs |
| `--effort <lvl>` | string | `config.defaultEffort` (`off`) | "off" always works; every other value is PROVIDER-NATIVE (see `nexus effort <provider>`) (shares `agent --role`'s OODA path); unsupported providers warn on stderr and run without it |
...
| `-o, --output <mode>` | enum | `text` | `text`, `json`, or `ndjson` |
| `--effort <lvl>` | string | `config.defaultEffort` (`off`) | "off" always works; every other value is PROVIDER-NATIVE (see `nexus effort <provider>`) applied to every lane; a lane whose provider can't honor it warns on stderr independently and runs without it |
...
| `-o, --output <mode>` | enum | `text` | `text`, `json`, or `ndjson` |
| `--effort <lvl>` | string | `config.defaultEffort` (`off`) | "off" always works; every other value is PROVIDER-NATIVE (see `nexus effort <provider>`) applied to every stage; a stage whose provider can't honor it warns on stderr independently and runs without it |
