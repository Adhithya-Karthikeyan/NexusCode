...
| --- | --- | --- |
| `gemini`, `bedrock`, `vertex` | Yes | Effort maps to an extended-thinking token budget (`low`=4000, `medium`=10000, `high`=24000) sent as `reasoning.budget_tokens`. |
| `anthropic` | **Depends on how you signed in** | With a **console API key**, yes — the budget is sent as `thinking.budget_tokens`, and `max_tokens` is raised above it automatically (the API requires `max_tokens` to exceed the thinking budget). With a **Claude.ai subscription OAuth sign-in** (`nexus login anthropic` browser flow), **no**: that credential accepts a `thinking` block, returns `200`, and silently omits all thinking content. Verified by capturing the literal request bytes on a live account. Rather than send a field the server ignores, the adapter omits it and prints one notice per process naming the two real options — sign in with an API key, or use the `claude-code` provider. `nexus effort anthropic` still lists the levels because they are real for an API key. |
...
| `claude-code`, `codex` | **Yes — and they always reason** | Both are forwarded now: `claude --effort <level>` and `codex -c model_reasoning_effort=<level>`. Each has its OWN scale, discovered live rather than assumed, and the two scales are genuinely different — run `nexus effort claude-code` (`low, medium, high, xhigh, max, ultracode, auto`, probed from `claude -p "/effort"`) or `nexus effort codex` (`low, medium, high, xhigh, max, ultra`, probed from `codex debug models --bundled`, which also reports each level's description and that model's own default). Critically, **neither has an "off"**: they reason by default and `--effort` selects the LEVEL, so `off` is omitted from their option lists entirely rather than offered as a no-op. Do not assume one provider's scale is valid for another — `xhigh` is real for `claude-code` and meaningless for `anthropic`, which accepts only `low\|medium\|high`. |
