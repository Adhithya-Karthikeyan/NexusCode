...
export function envOverrides(env: NodeJS.ProcessEnv): NexusConfigInput {
  ...
  // Any non-empty value is accepted — the legal set is provider-dependent
  // now (claude-code alone accepts seven names the old closed enum did not),
  // so this env var can carry a provider-native level like "xhigh" just as
  // legitimately as "off"/"medium". See `NexusConfig.defaultEffort`'s doc.
  const effort = env["NEXUS_DEFAULT_EFFORT"];
  if (effort && effort.trim().length > 0) {
    out.defaultEffort = effort.trim();
  }
